...
async def scan_wallets_parallel(seed_phrase, update, context):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text="🔑 Deriving wallet addresses...")
        ...
        await context.bot.send_message(
            ...
            text=f"🔍 Scanning {MAX_WALLETS} wallets for SOL and SPL tokens..."
        )
        ...
        async with aiohttp.ClientSession() as session:
            for batch_start in range(0, MAX_WALLETS, batch_size):
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text=f"📊 Progress: {batch_end}/{MAX_WALLETS} wallets... (Found: {found_count})"
                )
